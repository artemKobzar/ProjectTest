...
namespace ProjectTest.Application.DTOs.UserDto.Validators
{
    public class CreateUserValidator: AbstractValidator<CreateUserDto>
    {
        public CreateUserValidator()
        {
            ...
            RuleFor(u => u.Email).EmailAddress().NotEmpty().WithMessage("{PropertyName} is required").WithMessage("{PropertyName} must not exceed 200 characters").Length(1, 200).NotNull();
        }
    }
}
